...
export const createBooking = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    if (req.user!.role === 'viewer') {
      res.status(403).json({ error: 'Viewers do not have permission to book facilities' }); return;
    }
...
export const getMyBookings = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const bookings = await Booking.find(filter)
      .populate('facilityId', 'id name location type images')
      .sort({ date: 1 });
...
export const getAllBookings = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const bookings = await Booking.find(filter)
      .populate('facilityId', 'id name location type')
      .populate('userId',     'id name email role department')
      .sort({ createdAt: -1 });
...
export const updateBookingStatus = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    // Attach approval info to response so frontend can display approver name immediately
    const approval = await Approval.findOne({ bookingId: id }).populate('approvedById', 'name role');

    // Get facility name for notification (populate separately since booking was fetched without it)
    const facility = await Facility.findById(booking.facilityId).select('name').lean();
    const facilityName = (facility as any)?.name ?? 'facility';
